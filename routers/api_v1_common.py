...
from utils.rag_v1_common import process_embedding, search_embeddings, delete_embeddings
import shutil
import os
import time
...
def cm_search(
    ...
    result = search_embeddings(userId, question, collection_name, 
                fileIds = fileIds, n_results = n_results)
...
def cm_delete(
    ...
    with MysqlClient() as client:
        ...
        for item in list:
            max_seq = item['summaryChunkCount']
            if max_seq is None or max_seq == 0:
                max_seq = item['charChunkCount']
            
            # 임베딩된 ID 만듬
            embIds = []
            for i in range(max_seq):
                embIds.append(item['fileId']+'_'+str(i).rjust(4, '0'))

            # 임베딩 삭제
            delete_embeddings(embIds, collection_name)
